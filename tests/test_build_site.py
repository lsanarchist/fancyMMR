...
def byte_ranks_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    ranked_items = [
        (
            str(artifact["site_path"]),
            int(artifact["bytes"]),
        )
        for artifact in items
        if isinstance(artifact, dict)
    ...
    rank_count = len(ranked_items)
    rank_width = max(2, len(str(rank_count or 1)))
    return {
        site_path: f"R{index:0{rank_width}d}/{rank_count:0{rank_width}d}"
        for index, (site_path, _) in enumerate(
            sorted(ranked_items, key=lambda pair: (-pair[1], pair[0].lower())),
            start=1,
        )
    }


def format_ranks_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    ...
    for artifact in items:
        ...
        artifact_bytes = artifact.get("bytes")
        if not isinstance(site_path, str) or not isinstance(artifact_bytes, int):
            continue
        artifact_format = str(artifact.get("format") or "").strip().lower() or "other"
        grouped_items.setdefault(artifact_format, []).append((site_path, artifact_bytes))
    format_ranks: dict[str, str] = {}
    for artifact_format, ranked_items in grouped_items.items():
        format_count = len(ranked_items)
        format_width = max(2, len(str(format_count or 1)))
        format_ranks.update(
            {
                site_path: f"{artifact_format.upper()} {index:0{format_width}d}/{format_count:0{format_width}d}"
                for index, (site_path, _) in enumerate(
                    sorted(ranked_items, key=lambda pair: (-pair[1], pair[0].lower())),
                    start=1,
                )
...
def format_shares_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    grouped_items: dict[str, list[tuple[str, int]]] = {}
    for artifact in items:
        if not isinstance(artifact, dict):
            continue
        ...
        artifact_format = str(artifact.get("format") or "").strip().lower() or "other"
        grouped_items.setdefault(artifact_format, []).append((site_path, artifact_bytes))
    format_shares: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        format_total_bytes = sum(artifact_bytes for _, artifact_bytes in grouped_format_items)
        format_shares.update(
            {
                site_path: f"{artifact_format.upper()} {format_byte_share(artifact_bytes, format_total_bytes)}"
                for site_path, artifact_bytes in grouped_format_items
            }
        )
    return format_shares


def format_totals_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    grouped_items: dict[str, list[tuple[str, int]]] = {}
    for artifact in items:
        if not isinstance(artifact, dict):
            continue
        ...
        artifact_format = str(artifact.get("format") or "").strip().lower() or "other"
        grouped_items.setdefault(artifact_format, []).append((site_path, artifact_bytes))
    format_totals: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        format_total_bytes = sum(artifact_bytes for _, artifact_bytes in grouped_format_items)
        format_totals.update(
            {
                site_path: f"{artifact_format.upper()} {format_byte_count(format_total_bytes)}"
                for site_path, _ in grouped_format_items
            }
        )
    return format_totals


def format_counts_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    ...
    format_counts: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        format_counts.update(
            {
                site_path: f"{artifact_format.upper()} {count_label(len(grouped_format_items), 'file')}"
...
def format_file_shares_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    grouped_items: dict[str, list[str]] = {}
    ...
    format_file_shares: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        format_file_shares.update(
            {
                site_path: f"{artifact_format.upper()} {format_count_share(len(grouped_format_items), section_item_count)}"
                for site_path in grouped_format_items
            }
        )
    return format_file_shares


def format_section_byte_shares_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    ...
    format_section_byte_shares: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        format_total_bytes = sum(artifact_bytes or 0 for _, artifact_bytes in grouped_format_items)
        format_section_byte_shares.update(
            {
                site_path: f"{artifact_format.upper()} {format_byte_share(format_total_bytes, section_total_bytes)}"
                for site_path, _ in grouped_format_items
            }
        )
...
def format_section_file_ranks_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    grouped_items: dict[str, list[str]] = {}
    for artifact in items:
        if not isinstance(artifact, dict):
            continue
    ...
    format_rank_lookup = {
        ...
        for rank, artifact_format in enumerate(
            sorted(grouped_items, key=lambda key: (-len(grouped_items[key]), key)),
            start=1,
        )
    }
    format_section_file_ranks: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        format_section_file_ranks.update(
            {
                site_path: format_rank_lookup[artifact_format]
                for site_path in grouped_format_items
            }
        )
    return format_section_file_ranks


def format_byte_file_share_gaps_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    grouped_items: dict[str, list[tuple[str, int | None]]] = {}
    section_total_bytes = 0
    section_item_count = 0
    for artifact in items:
        ...
        if isinstance(normalized_bytes, int):
            section_total_bytes += normalized_bytes
        section_item_count += 1
    format_byte_file_share_gaps: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        format_total_bytes = sum(artifact_bytes or 0 for _, artifact_bytes in grouped_format_items)
        format_byte_file_share_gaps.update(
            {
                site_path: f"{artifact_format.upper()} {format_byte_file_share_gap(format_total_bytes, section_total_bytes, len(grouped_format_items), section_item_count)}"
                for site_path, _ in grouped_format_items
            }
        )
...
def format_average_sizes_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    ...
    format_average_sizes: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        total_bytes = sum(artifact_bytes or 0 for _, artifact_bytes in grouped_format_items)
        format_average_sizes.update(
            {
                site_path: f"{artifact_format.upper()} {format_average_byte_count(total_bytes, len(grouped_format_items))}"
                for site_path, _ in grouped_format_items
            }
        )
...
def format_max_to_median_ratios_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    grouped_items: dict[str, list[tuple[str, int | None]]] = {}
    for artifact in items:
        if not isinstance(artifact, dict):
            continue
    ...
    format_max_median_ratios: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        byte_values = [
            artifact_bytes
            for _, artifact_bytes in grouped_format_items
        ...
        median_bytes = median_byte_value(byte_values)
        format_max_median_ratios.update(
            {
                site_path: f"{artifact_format.upper()} {format_max_to_median_ratio(max_bytes, median_bytes)}"
                for site_path, _ in grouped_format_items
            }
        )
    return format_max_median_ratios


def format_top_shares_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    grouped_items: dict[str, list[tuple[str, int | None]]] = {}
    for artifact in items:
        if not isinstance(artifact, dict):
            continue
    ...
    format_top_shares: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        byte_values = [
            artifact_bytes
            for _, artifact_bytes in grouped_format_items
        ...
        format_top_shares.update(
            {
                site_path: f"{artifact_format.upper()} {format_top_file_share(max_bytes, total_bytes)}"
                for site_path, _ in grouped_format_items
            }
        )
    return format_top_shares


def format_smallest_shares_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    grouped_items: dict[str, list[tuple[str, int | None]]] = {}
    for artifact in items:
        if not isinstance(artifact, dict):
            continue
    ...
    format_smallest_shares: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        byte_values = [
            artifact_bytes
            for _, artifact_bytes in grouped_format_items
        ...
        min_bytes = min(byte_values) if byte_values else None
        total_bytes = sum(byte_values)
        format_smallest_shares.update(
            {
                site_path: f"{artifact_format.upper()} {format_smallest_file_share(min_bytes, total_bytes)}"
                for site_path, _ in grouped_format_items
            }
        )
    return format_smallest_shares


def format_share_gaps_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    grouped_items: dict[str, list[tuple[str, int | None]]] = {}
    for artifact in items:
        if not isinstance(artifact, dict):
            continue
    ...
    format_share_gaps_by_path: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        byte_values = [
            artifact_bytes
            for _, artifact_bytes in grouped_format_items
        ...
        max_bytes = max(byte_values) if byte_values else None
        total_bytes = sum(byte_values)
        format_share_gaps_by_path.update(
            {
                site_path: f"{artifact_format.upper()} {format_share_gap(min_bytes, max_bytes, total_bytes)}"
                for site_path, _ in grouped_format_items
            }
        )
    return format_share_gaps_by_path


def format_byte_deltas_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    grouped_items: dict[str, list[tuple[str, int | None]]] = {}
    for artifact in items:
        if not isinstance(artifact, dict):
            continue
    ...
    format_byte_deltas_by_path: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        byte_values = [
            artifact_bytes
            for _, artifact_bytes in grouped_format_items
        ...
        min_bytes = min(byte_values) if byte_values else None
        max_bytes = max(byte_values) if byte_values else None
        format_byte_deltas_by_path.update(
            {
                site_path: f"{artifact_format.upper()} {format_byte_delta(min_bytes, max_bytes)}"
                for site_path, _ in grouped_format_items
            }
        )
    return format_byte_deltas_by_path


def format_byte_ranges_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    grouped_items: dict[str, list[tuple[str, int | None]]] = {}
    for artifact in items:
        if not isinstance(artifact, dict):
            continue
    ...
    format_byte_ranges_by_path: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        byte_values = [
            artifact_bytes
            for _, artifact_bytes in grouped_format_items
        ...
        min_bytes = min(byte_values) if byte_values else None
        max_bytes = max(byte_values) if byte_values else None
        format_byte_ranges_by_path.update(
            {
                site_path: f"{artifact_format.upper()} {format_byte_range(min_bytes, max_bytes)}"
                for site_path, _ in grouped_format_items
            }
        )
    return format_byte_ranges_by_path


def format_spread_ratios_by_site_path(items: list[dict[str, object]]) -> dict[str, str]:
    grouped_items: dict[str, list[tuple[str, int | None]]] = {}
    for artifact in items:
        if not isinstance(artifact, dict):
            continue
    ...
    format_spread_ratios_by_path: dict[str, str] = {}
    for artifact_format, grouped_format_items in grouped_items.items():
        byte_values = [
            artifact_bytes
            for _, artifact_bytes in grouped_format_items
        ...
        min_bytes = min(byte_values) if byte_values else None
        max_bytes = max(byte_values) if byte_values else None
        format_spread_ratios_by_path.update(
            {
                site_path: f"{artifact_format.upper()} {format_byte_spread_ratio(min_bytes, max_bytes)}"
                for site_path, _ in grouped_format_items
            }
        )
    return format_spread_ratios_by_path
...
def test_build_site_outputs_pages_assets_and_copied_json(tmp_path: Path) -> None:
    ...
    publication_link_byte_ranks = byte_ranks_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    publication_link_format_ranks = format_ranks_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    publication_link_format_shares = format_shares_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    publication_link_format_totals = format_totals_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    publication_link_format_counts = format_counts_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    publication_link_format_file_shares = format_file_shares_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    ...
    publication_link_format_max_median_ratios = format_max_to_median_ratios_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    publication_link_format_top_shares = format_top_shares_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    publication_link_format_smallest_shares = format_smallest_shares_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    publication_link_format_share_gaps = format_share_gaps_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    publication_link_format_byte_deltas = format_byte_deltas_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    publication_link_format_byte_ranges = format_byte_ranges_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    publication_link_format_spread_ratios = format_spread_ratios_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    ...
    publication_link_format_section_file_ranks = format_section_file_ranks_by_site_path(
        manifest_generated_download_items(workspace, pipeline_manifest)
    )
    ...
    staged_link_byte_ranks = byte_ranks_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    staged_link_format_ranks = format_ranks_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    staged_link_format_shares = format_shares_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    staged_link_format_totals = format_totals_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    staged_link_format_counts = format_counts_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    staged_link_format_file_shares = format_file_shares_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    ...
    staged_link_format_max_median_ratios = format_max_to_median_ratios_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    staged_link_format_top_shares = format_top_shares_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    staged_link_format_smallest_shares = format_smallest_shares_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    staged_link_format_share_gaps = format_share_gaps_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    staged_link_format_byte_deltas = format_byte_deltas_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    staged_link_format_byte_ranges = format_byte_ranges_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    staged_link_format_spread_ratios = format_spread_ratios_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    ...
    staged_link_format_section_file_ranks = format_section_file_ranks_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_staged_artifacts"])
    )
    ...
    fetch_failure_link_byte_ranks = byte_ranks_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    fetch_failure_link_format_ranks = format_ranks_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    fetch_failure_link_format_shares = format_shares_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    fetch_failure_link_format_totals = format_totals_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    fetch_failure_link_format_counts = format_counts_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    fetch_failure_link_format_file_shares = format_file_shares_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    ...
    fetch_failure_link_format_max_median_ratios = format_max_to_median_ratios_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    fetch_failure_link_format_top_shares = format_top_shares_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    fetch_failure_link_format_smallest_shares = format_smallest_shares_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    fetch_failure_link_format_share_gaps = format_share_gaps_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    fetch_failure_link_format_byte_deltas = format_byte_deltas_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    fetch_failure_link_format_byte_ranges = format_byte_ranges_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    fetch_failure_link_format_spread_ratios = format_spread_ratios_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    ...
    fetch_failure_link_format_section_file_ranks = format_section_file_ranks_by_site_path(
        list(pipeline_manifest["source_pipeline_diagnostics"]["downloadable_fetch_failure_artifacts"])
    )
    ...
    assert (
        f'class="output-registry-badge output-registry-badge-rank">'
        f'{publication_link_byte_ranks["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-rank">'
        f'{publication_link_byte_ranks["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-rank">'
        f'{publication_link_format_ranks["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-rank">'
        f'{publication_link_format_ranks["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share">'
        f'{publication_link_format_shares["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share">'
        f'{publication_link_format_shares["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-total">'
        f'{publication_link_format_totals["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-total">'
        f'{publication_link_format_totals["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-count">'
        f'{publication_link_format_counts["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-count">'
        f'{publication_link_format_counts["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-file-share">'
        f'{publication_link_format_file_shares["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-file-share">'
        f'{publication_link_format_file_shares["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-average-size">'
        f'{publication_link_format_average_sizes["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-average-size">'
    ...
    assert (
        f'class="output-registry-badge output-registry-badge-format-max-median">'
        f'{publication_link_format_max_median_ratios["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-max-median">'
        f'{publication_link_format_max_median_ratios["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-top-share">'
        f'{publication_link_format_top_shares["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-top-share">'
        f'{publication_link_format_top_shares["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-smallest-share">'
        f'{publication_link_format_smallest_shares["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-smallest-share">'
        f'{publication_link_format_smallest_shares["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share-gap">'
        f'{publication_link_format_share_gaps["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share-gap">'
        f'{publication_link_format_share_gaps["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-delta">'
        f'{publication_link_format_byte_deltas["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-delta">'
        f'{publication_link_format_byte_deltas["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-range">'
        f'{publication_link_format_byte_ranges["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-range">'
        f'{publication_link_format_byte_ranges["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-spread-ratio">'
        f'{publication_link_format_spread_ratios["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-spread-ratio">'
        f'{publication_link_format_spread_ratios["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-byte-share">'
        f'{publication_link_format_section_byte_shares["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-byte-share">'
    ...
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-file-rank">'
        f'{publication_link_format_section_file_ranks["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-file-rank">'
        f'{publication_link_format_section_file_ranks["data/business_model_summary.csv"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-file-share-gap">'
        f'{publication_link_format_byte_file_share_gaps["data/metrics.json"]}<'
    ) in publication_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-file-share-gap">'
    ...
    assert (
        f'class="output-registry-badge output-registry-badge-rank">'
        f'{staged_link_byte_ranks["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-rank">'
        f'{staged_link_byte_ranks["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-rank">'
        f'{staged_link_format_ranks["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-rank">'
        f'{staged_link_format_ranks["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share">'
        f'{staged_link_format_shares["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share">'
        f'{staged_link_format_shares["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-total">'
        f'{staged_link_format_totals["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-total">'
        f'{staged_link_format_totals["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-count">'
        f'{staged_link_format_counts["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-count">'
        f'{staged_link_format_counts["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-file-share">'
        f'{staged_link_format_file_shares["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-file-share">'
        f'{staged_link_format_file_shares["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-average-size">'
        f'{staged_link_format_average_sizes["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-average-size">'
    ...
    assert (
        f'class="output-registry-badge output-registry-badge-format-max-median">'
        f'{staged_link_format_max_median_ratios["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-max-median">'
        f'{staged_link_format_max_median_ratios["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-top-share">'
        f'{staged_link_format_top_shares["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-top-share">'
        f'{staged_link_format_top_shares["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-smallest-share">'
        f'{staged_link_format_smallest_shares["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-smallest-share">'
        f'{staged_link_format_smallest_shares["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share-gap">'
        f'{staged_link_format_share_gaps["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share-gap">'
        f'{staged_link_format_share_gaps["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-delta">'
        f'{staged_link_format_byte_deltas["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-delta">'
        f'{staged_link_format_byte_deltas["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-range">'
        f'{staged_link_format_byte_ranges["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-range">'
        f'{staged_link_format_byte_ranges["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-spread-ratio">'
        f'{staged_link_format_spread_ratios["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-spread-ratio">'
        f'{staged_link_format_spread_ratios["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-byte-share">'
        f'{staged_link_format_section_byte_shares["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-byte-share">'
    ...
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-file-rank">'
        f'{staged_link_format_section_file_ranks["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-file-rank">'
        f'{staged_link_format_section_file_ranks["data/source_pipeline/snapshots/run_manifest.json"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-file-share-gap">'
        f'{staged_link_format_byte_file_share_gaps["data/source_pipeline/processed/detail_page_rows.csv"]}<'
    ) in staged_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-file-share-gap">'
    ...
    assert ".output-registry-badge-rank {" in site_css
    assert ".output-registry-badge-format-rank {" in site_css
    assert ".output-registry-badge-format-share {" in site_css
    assert ".output-registry-badge-format-total {" in site_css
    ...
    assert ".output-registry-badge-format-max-median {" in site_css
    assert ".output-registry-badge-format-top-share {" in site_css
    assert ".output-registry-badge-format-smallest-share {" in site_css
    assert ".output-registry-badge-format-share-gap {" in site_css
    assert ".output-registry-badge-format-byte-delta {" in site_css
...
def test_build_site_copies_manifest_driven_fetch_failure_downloads(tmp_path: Path) -> None:
    ...
    assert (
        f'class="output-registry-badge output-registry-badge-rank">'
        f'{fetch_failure_link_byte_ranks["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-rank">'
        f'{fetch_failure_link_byte_ranks["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-rank">'
        f'{fetch_failure_link_byte_ranks["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-rank">'
        f'{fetch_failure_link_format_ranks["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-rank">'
        f'{fetch_failure_link_format_ranks["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-rank">'
        f'{fetch_failure_link_format_ranks["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share">'
        f'{fetch_failure_link_format_shares["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share">'
        f'{fetch_failure_link_format_shares["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share">'
        f'{fetch_failure_link_format_shares["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-total">'
        f'{fetch_failure_link_format_totals["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-total">'
        f'{fetch_failure_link_format_totals["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-total">'
        f'{fetch_failure_link_format_totals["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-count">'
        f'{fetch_failure_link_format_counts["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-count">'
        f'{fetch_failure_link_format_counts["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-count">'
        f'{fetch_failure_link_format_counts["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-file-share">'
        f'{fetch_failure_link_format_file_shares["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-file-share">'
        f'{fetch_failure_link_format_file_shares["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-file-share">'
        f'{fetch_failure_link_format_file_shares["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-average-size">'
        f'{fetch_failure_link_format_average_sizes["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-average-size">'
        f'{fetch_failure_link_format_average_sizes["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-average-size">'
    ...
    assert (
        f'class="output-registry-badge output-registry-badge-format-max-median">'
        f'{fetch_failure_link_format_max_median_ratios["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-max-median">'
        f'{fetch_failure_link_format_max_median_ratios["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-max-median">'
        f'{fetch_failure_link_format_max_median_ratios["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-top-share">'
        f'{fetch_failure_link_format_top_shares["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-top-share">'
        f'{fetch_failure_link_format_top_shares["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-top-share">'
        f'{fetch_failure_link_format_top_shares["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-smallest-share">'
        f'{fetch_failure_link_format_smallest_shares["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-smallest-share">'
        f'{fetch_failure_link_format_smallest_shares["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-smallest-share">'
        f'{fetch_failure_link_format_smallest_shares["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share-gap">'
        f'{fetch_failure_link_format_share_gaps["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share-gap">'
        f'{fetch_failure_link_format_share_gaps["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-share-gap">'
        f'{fetch_failure_link_format_share_gaps["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-delta">'
        f'{fetch_failure_link_format_byte_deltas["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-delta">'
        f'{fetch_failure_link_format_byte_deltas["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-delta">'
        f'{fetch_failure_link_format_byte_deltas["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-range">'
        f'{fetch_failure_link_format_byte_ranges["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-range">'
        f'{fetch_failure_link_format_byte_ranges["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-range">'
        f'{fetch_failure_link_format_byte_ranges["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-spread-ratio">'
        f'{fetch_failure_link_format_spread_ratios["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-spread-ratio">'
        f'{fetch_failure_link_format_spread_ratios["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-spread-ratio">'
        f'{fetch_failure_link_format_spread_ratios["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-byte-share">'
        f'{fetch_failure_link_format_section_byte_shares["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-byte-share">'
        f'{fetch_failure_link_format_section_byte_shares["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-byte-share">'
    ...
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-file-rank">'
        f'{fetch_failure_link_format_section_file_ranks["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-file-rank">'
        f'{fetch_failure_link_format_section_file_ranks["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-section-file-rank">'
        f'{fetch_failure_link_format_section_file_ranks["data/fetch_failures/category--sales.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-file-share-gap">'
        f'{fetch_failure_link_format_byte_file_share_gaps["data/fetch_failures/category--ai.html"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-file-share-gap">'
        f'{fetch_failure_link_format_byte_file_share_gaps["data/fetch_failures/category--ai.json"]}<'
    ) in fetch_failure_output_section
    assert (
        f'class="output-registry-badge output-registry-badge-format-byte-file-share-gap">'
