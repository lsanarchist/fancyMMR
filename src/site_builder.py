...
def output_registry_link_markup(item: dict[str, object]) -> str:
    ...
    rank_badge_html = (
        f'<span class="output-registry-badge output-registry-badge-rank">{html.escape(item_byte_rank)}</span>'
        if item_byte_rank
        else ""
    )
    format_rank_badge_html = (
        f'<span class="output-registry-badge output-registry-badge-format-rank">{html.escape(item_format_rank)}</span>'
        if item_format_rank
        else ""
    )
    ...
    format_section_byte_rank_badge_html = (
        f'<span class="output-registry-badge output-registry-badge-format-section-byte-rank">{html.escape(item_format_section_byte_rank)}</span>'
...
def global_output_command_items(download_items: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    ranked_site_paths = [
        (
            str(artifact.get("site_path") or ""),
            int(artifact.get("bytes")),
        )
        for artifact in download_items
        if str(artifact.get("site_path") or "") and isinstance(artifact.get("bytes"), int)
    ]
    rank_count = len(ranked_site_paths)
    rank_width = max(2, len(str(rank_count or 1)))
    byte_rank_lookup = {
        site_path: f"R{rank:0{rank_width}d}/{rank_count:0{rank_width}d}"
        for rank, (site_path, _) in enumerate(
            sorted(
                ranked_site_paths,
    ...
    for artifact_format, ranked_items in format_ranked_site_paths.items():
        ...
        format_rank_lookup.update(
            {
                site_path: f"{artifact_format.upper()} {rank:0{format_width}d}/{format_count:0{format_width}d}"
                for rank, (site_path, _) in enumerate(
    ...
    format_lane_count = len(format_item_counts)
    format_lane_width = max(2, len(str(format_lane_count or 1)))
    format_section_byte_rank_lookup = {
...
.rail-command-divider-smallest-share {
  font-size: 0.64rem;
  letter-spacing: 0.08em;
  color: var(--ink-soft);
}
...
.output-registry-badge-rank {
  color: var(--warning);
  border-color: rgba(255, 191, 97, 0.22);
  background: rgba(255, 191, 97, 0.08);
}

.output-registry-badge-format-rank {
  color: var(--cyan);
  border-color: rgba(98, 201, 214, 0.22);
  background: rgba(98, 201, 214, 0.08);
}
...
.output-registry-badge-format-section-byte-rank {
  color: var(--cyan);
  border-color: rgba(98, 201, 214, 0.22);
  background: rgba(98, 201, 214, 0.12);
}
